...
from pypastml.ml import MAP, JC
...
class ACRStateTestMAPJC(unittest.TestCase):
    ...
    def setUp(self):
        ...
        acr(self.tree, self.df, prediction_method=MAP, model=JC)

    def test_collapsed_vs_full(self):
        tree = read_tree(TREE_NWK)
        acr(tree, self.df, prediction_method=MAP, model=JC)
